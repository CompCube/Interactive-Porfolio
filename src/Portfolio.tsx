  ...
  {id:"vfx",label:"VFX / Shaders",icon:"✨",hex:"#3C226B",orbitRadius:29,orbitSpeed:.0017,startAngle:2.5,radius:1.08,rings:true,desc:"Real-time VFX and custom shaders.",moons:[
    {id:"magic-barrier",label:"Magic Barrier",icon:"🛡️",orbitRadius:2.9,orbitSpeed:.011,startAngle:.5,inclination:.42,radius:.28,hex:"#9e7292",
      ...
      categories:[
        ...
        {id:"technical",label:"Breakdown",caption:"",imgs:[
          {label:"Mesh",src:gh("vfx/magic-barrier/technical/01-mesh.png"),bg:"radial-gradient(ellipse at 50% 40%,#14041a,#0a020c)",caption:"The shield uses a **double-sided transparent mesh** whose silhouette defines the readability of the entire effect. Its topology was designed to support both the Fresnel highlights and the vertex displacement, ensuring that the pulse animation remains smooth and visually consistent from every angle."},
          {label:"UV Layout",src:gh("vfx/magic-barrier/technical/02-UV.png"),bg:"radial-gradient(ellipse at 45% 55%,#160418,#0a0210)",caption:"The UVs were **manually adjusted in Photoshop** to control how textures wrap around the sphere. This provided precise placement of the equatorial energy band and ensured that surface details tiled consistently before any shader work was introduced."},
          {label:"Shader Graph",src:gh("vfx/magic-barrier/technical/03-Shadegraph.png"),bg:"radial-gradient(ellipse at 55% 45%,#12041a,#08020c)",caption:"A **single-pass shader** handles both sides of the mesh using **IsFrontFace**, eliminating the need for duplicate materials or additional rendering passes. The exterior face generates the Fresnel rim and surface details, while the interior receives its own visual treatment. HDR values drive the rim and energy band into Unity's bloom pipeline, creating the shield's characteristic glow."},
          {label:"VFX Graph",src:gh("vfx/magic-barrier/technical/04-vfx-graph.png"),bg:"radial-gradient(ellipse at 50% 50%,#160218,#0c010c)",caption:"VFX Graph drives **two independent effects** within the same asset: vertex displacement for the breathing pulse and a second **Mesh Output** context responsible for the outer aura. The entire system is event-driven, spawning on demand and automatically despawning after six seconds, with no continuous particle simulation."},
        ],videoId:null},
      ]},
    {id:"waterfall",label:"Stylized Waterfall",icon:"💧",orbitRadius:3.8,orbitSpeed:.008,startAngle:3.2,inclination:-.3,radius:.22,hex:"#906080",
      ...
      categories:[
        ...
        {id:"technical",label:"Breakdown",caption:"",imgs:[
          {label:"Mesh",src:gh("vfx/waterfall/technical/01-mesh.png"),bg:"radial-gradient(ellipse at 55% 40%,#04101a,#020810)",caption:"The waterfall body is built from a **single low-complexity mesh** designed to let the shader carry most of the visual detail. Its topology follows the direction of the water flow, minimizing UV stretching and preserving smooth motion around curves."},
          {label:"Waterfall Shader",src:gh("vfx/waterfall/technical/02-waterfall-shader.png"),bg:"radial-gradient(ellipse at 45% 60%,#040e18,#020708)",caption:"The water surface is generated entirely in **Shader Graph** using two texture layers scrolling at different speeds to simulate flow without relying on physics. Surface normals are reconstructed in real time through **NormalFromHeight**, removing the need for baked normal maps. Key parameters such as colour, tiling, foam intensity, and scroll speed are exposed for live tweaking."},
          {label:"Splash Shader",src:gh("vfx/waterfall/technical/03-splash-shader.png"),bg:"radial-gradient(ellipse at 50% 50%,#041218,#020a0c)",caption:"A dedicated **Unlit Shader** powers the splash and fog particles. A soft radial texture combined with a **noise-based alpha clip** produces natural dissolving edges without the cost of complex transparency. With no lighting model or normal maps, the shader stays lightweight enough to support hundreds of overlapping particles while remaining suitable for mobile hardware."},
          {label:"Particle System",src:gh("vfx/waterfall/technical/04-particle-system.png"),bg:"radial-gradient(ellipse at 55% 55%,#040e16,#020810)",caption:"Splash and fog are handled through a lightweight Particle System composed of **1000 quad particles**, built in **VFX Graph** rather than the legacy Shuriken system for better performance on mobile. Each particle uses the splash shader's noise-based alpha clip for a soft dissolving edge, while the whole system stays intentionally simple to limit overdraw in areas with heavy particle overlap."},
        ],videoId:null},
      ]},
  ]},
    ...
    {id:"scatter-tool",label:"Replacement & Scatter Tool",icon:"🔧",orbitRadius:2.1,orbitSpeed:.012,startAngle:2,inclination:-.28,radius:.26,hex:"#9099e2",
      ...
      categories:[
        ...
        {id:"ui-architecture",label:"UI Architecture",hex:"#6070d8",
         ...
         subcategories:[
           {id:"ui-overview",label:"UI Overview",imgs:[{label:"UI Architecture",src:null,bg:"radial-gradient(ellipse at 50% 40%,#0a0e1a,#050810)",caption:"The tool is implemented as a single Unity EditorWindow with modular UI sections. Each feature is exposed through collapsible panels, separating target configuration, prefab management, transform controls, and execution logic. The interface prioritizes clarity and fast iteration over deep nesting or complex menus."}]},
         ]},
      ]},
  ]},
