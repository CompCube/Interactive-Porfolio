          ...
          <div>
            {imgs[imgIdx]?.compare?(
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"1rem",marginBottom:".75rem"}}>
                ...
                {imgs.length>1&&(<div style={{gridColumn:"1 / -1",display:"flex",justifyContent:"space-between"}}>
                  <button onClick={()=>setImgIdx(i=>(i-1+imgs.length)%imgs.length)} style={{background:"none",border:"none",color:c,fontSize:".72rem",fontFamily:"'JetBrains Mono',monospace",cursor:"pointer"}}>‹ Prev</button>
                  <button onClick={()=>setImgIdx(i=>(i+1)%imgs.length)} style={{background:"none",border:"none",color:c,fontSize:".72rem",fontFamily:"'JetBrains Mono',monospace",cursor:"pointer"}}>Next ›</button>
                </div>)}
              </div>
