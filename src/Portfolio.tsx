  ...
  {id:"game-art",label:"Game Art",icon:"🎨",hex:"#ff99cc",
   ...
   subcategories:[
     {id:"references-identity",label:"References & Identity",imgs:[
       {label:"Moodboard",src:gh("games/hollow-end/game-art/references-identity/01-moodboard.png"),bg:"radial-gradient(ellipse at 50% 50%,#1a0c14,#0a060a)",caption:"The moodboard combines real-world transit architecture with liminal and institutional references, exploring repetition, scale, and controlled lighting. Created before production began, it helped define both the structural language of the environment and the overall atmosphere of Hollow End."},
       {label:"Reference Images",src:gh("games/hollow-end/game-art/references-identity/02-reference-images.png"),bg:"radial-gradient(ellipse at 45% 55%,#1a0e10,#0a0608)",caption:"During a trip to Berlin, I documented several U-Bahn stations through more than 300 reference photographs. These images guided the project throughout development, helping establish the aesthetic, scale, proportions, and architectural details of the station."},
     ...
     {id:"props",label:"Props",imgs:[
       ...
       {label:"Two-Version System",src:null,bg:"radial-gradient(ellipse at 45% 55%,#1a1004,#0e0802)",caption:"Every prop exists in two different states: New, used in the Backroom, and Abandoned, used in the realistic zone. While both versions share the same geometry, each required its own texturing pass, material setup, and visual language to match the atmosphere of its environment.",compare:{stacked:true,
         ...
         right:{label:"Abandoned",imgs:[
           {label:"Prop (Abandoned)",src:gh("games/hollow-end/game-art/props/02-props-old.png"),bg:"radial-gradient(ellipse at 50% 50%,#1a0c04,#0a0502)"},
           {label:"Sign (Abandoned)",src:gh("games/hollow-end/game-art/props/02-sign-old.png"),bg:"radial-gradient(ellipse at 50% 50%,#1a0c04,#0a0502)"},
         ]},
       }},
       {label:"Texture Atlasing & Material Efficiency",src:gh("games/hollow-end/game-art/props/03-atlas-1.png"),bg:"radial-gradient(ellipse at 55% 45%,#180e04,#0c0602)",caption:"To reduce draw calls and material count, props were grouped into shared texture atlases instead of receiving unique materials. In total, 35 custom assets use only 7 materials, while 45 imported props were condensed into 15, resulting in 90 prefabs across both environments."},
       {label:"Texture Atlasing & Material Efficiency",src:gh("games/hollow-end/game-art/props/03-atlas-2.png"),bg:"radial-gradient(ellipse at 55% 45%,#180e04,#0c0602)",caption:"To reduce draw calls and material count, props were grouped into shared texture atlases instead of receiving unique materials. In total, 35 custom assets use only 7 materials, while 45 imported props were condensed into 15, resulting in 90 prefabs across both environments."},
     ...
     {id:"customization",label:"Asset Customization & Easter Eggs",imgs:[
       {label:"Asset & Material Customization",src:gh("games/hollow-end/game-art/customization/01-material-customization.png"),bg:"radial-gradient(ellipse at 50% 50%,#100a1a,#06040e)",caption:"Assets and materials were heavily customized through layered dirt, dust, and debris generators, together with material parameter adjustments, to create a worn, believable look while maximizing visual variety across the environment."},
       {label:"Environmental Storytelling",src:gh("games/hollow-end/game-art/customization/02-enviromental-storytelling.png"),bg:"radial-gradient(ellipse at 45% 55%,#140a1c,#08040e)",caption:"Selected props feature traces of past events — blood stains, abandoned objects, and signs of struggle — hinting that others attempted to escape the station long before the player arrived."},
       {label:"Diegetic Signage",src:gh("games/hollow-end/game-art/customization/03-diegetic-signace.png"),bg:"radial-gradient(ellipse at 55% 45%,#0c0a1a,#04040e)",caption:"Signs, maps, and line indicators were redesigned to follow the station's internal logic, reinforcing both navigation and the overall visual identity of the environment."},
       {label:"Personal Easter Eggs",src:gh("games/hollow-end/game-art/customization/04-personal-easter-eggs.png"),bg:"radial-gradient(ellipse at 50% 60%,#100c1a,#06040e)",caption:"In order to add small personal details hidden throughout the environment, several station names and metro stops are subjects from my degree."},
     ]},
   ]},
  {id:"vfx",label:"VFX & Shaders",icon:"✨",hex:"#aaccff",
   ...
   subcategories:[
     {id:"shaders",label:"Shaders",imgs:[
       {label:"Retro Screen Shader",src:null,bg:bgSH,videoId:"ALOnC9RBBj8",caption:"Inspired by the vintage aesthetic commonly associated with Backrooms imagery, this shader adds a subtle VHS-like filter during specific moments in the Backroom. Beyond its visual style, it reinforces the feeling that the space is detached from reality."},
       ...
       {label:"Outline Shader",src:null,bg:"radial-gradient(ellipse at 55% 45%,#0a1422,#050a14)",videoId:"NR4iE6zesn4",caption:"Interactable objects are highlighted using an animated outline system that does not modify their original materials. Color, thickness, and animation change depending on the object's state, allowing players to distinguish available interactions from inaccessible ones at a glance."},
     ]},
  ...
  {id:"application-flow",label:"Application Flow",icon:"🧭",hex:"#3ec2e0",
   ...
   subcategories:[
     ...
     {id:"interview",label:"06 — Prepare for the Interview",imgs:[
       {label:"Interview",src:gh("ai-projects/06-interview-01.png"),bg:"radial-gradient(ellipse at 50% 50%,#091c1e,#04100e)",caption:"The application can **reuse the context of a completed application** to launch an interview session for that specific role. Instead of starting another conversation from scratch, the **Interview agent** receives the relevant application context and runs a **structured mock interview** based on the position and candidate."},
...
.hdr-link::after{content:"";position:absolute;left:.3rem;right:.3rem;bottom:.15rem;height:1.5px;background:#EDC32B;transform:scaleX(0);transform-origin:left;transition:transform .3s cubic-bezier(.2,.9,.3,1)}
.hdr-link:hover::after,.hdr-link:focus-visible::after{transform:scaleX(1)}
      ...
      {catId===null?(
        ...
        <div style={{padding:"0 1.75rem 1.75rem",display:"grid",gridTemplateColumns:"55fr 45fr",gap:"2rem"}}>
          <div>
            ...
            {caption&&(<div key={`${catId}-${subcatId}-${imgIdx}`} style={{marginTop:".2rem",animation:"captionFade .22s ease"}}><div style={{fontSize:".6rem",color:`${c}88`,fontFamily:"'JetBrains Mono',monospace",letterSpacing:".14em",marginBottom:".3rem"}}>{!imgs[imgIdx]?.compare&&(imgs[imgIdx]?.label||activeSubcat?.label)?.toUpperCase()}{!imgs[imgIdx]?.compare&&imgs.length>0?` · ${imgIdx+1}/${imgs.length}`:""}</div><p style={{fontSize:".82rem",lineHeight:1.65,color:"rgba(232,232,240,.72)",whiteSpace:"pre-line",textAlign:"left"}}>{renderBold(caption)}</p><HowItWorks text={imgs[imgIdx]?.details} c={c}/></div>)}
